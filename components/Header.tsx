...
export function Header() {
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-navy/95 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav aria-label="Primary navigation" className="hidden items-center gap-7 lg:flex">
          {navItems.map((item) => (
            <Link
              className="text-sm text-slate-300 transition hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-blue"
              href={item.href}
              key={item.href}
            >
              {item.label}
        ...
        <details className="relative lg:hidden">
          ...
          <div className="absolute right-0 mt-4 w-[min(88vw,22rem)] rounded-lg border border-white/10 bg-[#080d16] p-3 shadow-glow">
            <nav aria-label="Mobile navigation" className="grid gap-1">
              {navItems.map((item) => (
                <Link
                  className="rounded-md px-4 py-3 text-sm text-slate-300 transition hover:bg-white/[0.08] hover:text-white"
                  href={item.href}
                  key={item.href}
                >
                  {item.label}
              ...
              <Link
                className="mt-2 rounded-md bg-amber px-4 py-3 text-sm font-semibold uppercase tracking-[0.16em] text-white"
                href="/request-quote"
              >
                Request a Quote
              </Link>
